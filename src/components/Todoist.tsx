...
const Todoist: React.FC<TodoistProps> = ({ projectId, projectName, allProjects }) => {
  ...
  const moveMenu = (taskId: string) => (
    <Menu>
      {allProjects.map((project) => (
        <Menu.Item
          key={project.id}
          onClick={() => handleMoveTask(taskId, project.id)}
        >
          {project.name}
        </Menu.Item>
      ))}
  ...
  return (
    <div style={{ 
            ...
            <div style={{ display: "flex", flexDirection: "column", width: "100%" }}>
              <div style={{ display: "flex", alignItems: "center", width: "100%" }}>
                ...
                {editingTask?.id === task.id ? (
                  ...
                  <>
                    <span style={{ flex: 1, color: "black", fontWeight: "normal" }}>
                      {task.content}
                    </span>
                    <EditOutlined
                    ...
                    />
                    <Dropdown overlay={moveMenu(task.id)} trigger={["click"]}>
                      <SwapOutlined
                        style={{ cursor: "pointer", marginLeft: "8px", color: "green" }}
                      />
                    </Dropdown>
                    <DeleteOutlined
        ...
        locale={{
          emptyText: (
            <div style={{ textAlign: "center" }}>
              <img
                src="https://img.freepik.com/premium-vector/vector-person-painting_844724-17899.jpg?w=360"
                alt="No tasks"
                style={{ width: "150px", marginBottom: "16px" }}
              />
              <h3>Start small (or dream big ...)</h3>
              <p style={{ color: "grey" }}>
                Add a task or find a template to start with your project
              </p>
            </div>
          ),
        }}
      ...
      <Button
        type="text"
        icon={<PlusOutlined style={{ color: '#ff9933' }} />}
